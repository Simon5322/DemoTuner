...
class MsEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = self.reward_function(reward, throughput, latency, 'speed')
    ...
    def reward_function(self, reward, throughput, latency, reward_func):
    ...
    def _get_obs(self):
        ...
        for i in range(self.repeat_times):
            ...
            if not self.fake_test:
                ...
                self.temp_throughput, self.temp_latency = 1, 2
                print('!!!!!!!!!!!!!!!!!!!fake test!!!!!!!!!!!!!!!!!!!')
        ...
        # metricsD_before_task = [
        #     (metricsD_before_task_list[0][i] + metricsD_before_task_list[1][i] + metricsD_before_task_list[2][i]) / len(
        #         metricsD_before_task_list) for key, value in range(len(metricsD_before_task_list))]
        #
        # metricsD = [(metricsD_list[0][i] + metricsD_list[1][i] + metricsD_list[2][i]) / len(metricsD_list) for i in
        #             range(len(metricsD_list))]
        #
    ...
    def get_workload(self):
        continues_settings = {}
        discrete_settings = {}
        if isinstance(self.benchmark, YCSB):
            config = configparser.ConfigParser()
            config.read(r'/home/zhouyuxuan/workspace/pythonWorkspace/bertProject/config/pg.ini')

            workload_file_path = r'/home/zhouyuxuan/workspace/pythonWorkspace/bertProject/src/tuning_params_states/pg_workload.yml'
